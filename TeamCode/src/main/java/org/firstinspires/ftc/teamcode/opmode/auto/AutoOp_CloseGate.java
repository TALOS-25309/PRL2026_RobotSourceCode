...
import static org.firstinspires.ftc.teamcode.part.Constants.CLOSE_SHOOT_VELOCITY;
import static org.firstinspires.ftc.teamcode.part.Constants.EATER_FEED_TIME_MS;
import static org.firstinspires.ftc.teamcode.part.Constants.RAPID_FIRE_DELAY_CLOSE_MS;
...
/**
 * AutoOp_CloseGate 의 Schedule 기반 버전. (AutoOp_FarCorner 와 구조 동일)
 * <p>
 ...
 * <p>
 * FarCorner 와 다른 점은 "어디서 쏘고 어디서 먹느냐" 뿐입니다.
 * CloseZone 발사 지점 / 게이트에서 먹기 / 근거리 발사 속도.
 */
@Autonomous(name = "CloseZone+Gate (Schedule)", group = "Auto")
public class AutoOp_CloseGate extends LinearOpMode {
    ...
    public void runOpMode() {
        drive = new MecanumDrive(hardwareMap, CLOSE_START_POSE);
        ...
        drivePath(drive.actionBuilder(CLOSE_START_POSE)
    ...
    /** 인테이크 ON -> 게이트 이동 -> 먹는 동안 대기 -> 발사 지점 복귀 -> 2발 발사 -> 다음 사이클 */
    private void cycle(int i) {
        ...
        // 게이트 도착 -> EAT_GATE_WAIT_TIME 동안 먹기 -> 복귀
        afterPath(() -> Schedule.addTask(() -> {
            eater.stop();
            shooter.runShooter(CLOSE_SHOOT_VELOCITY);   // 복귀하면서 미리 예열
            drivePath(drive.actionBuilder(EAT_GATE_POSE)
                    .strafeToLinearHeading(CLOSE_SHOOT_POS, CLOSE_SHOOT_HEADING)
                    .build());
    ...
    private void aimAndShoot(Runnable next) {
        shooter.runShooter(CLOSE_SHOOT_VELOCITY);   // 조준하는 동안 예열 (조준 시간이 곧 예열 시간)
    ...
    private void shootTwice(Runnable next) {
        shooter.runShooter(CLOSE_SHOOT_VELOCITY);
        whenSpunUp(() -> {
            eater.feedToShooter();
            Schedule.addTask(() -> whenSpunUp(() -> {
                eater.feedToShooter();
                Schedule.addTask(next, EATER_FEED_TIME_MS / 1000.0);
            }), (EATER_FEED_TIME_MS + RAPID_FIRE_DELAY_CLOSE_MS) / 1000.0);
        });
    }
    ...
    private void whenSpunUp(Runnable task) {
        ...
        Schedule.addConditionalTask(task, Schedule.RUN_INSTANTLY, () ->
                Math.abs(shooter.getVelocity() - CLOSE_SHOOT_VELOCITY) < VEL_TOLERANCE
