...
public class Vision implements Part {
    ...
    public void init(HardwareMap hardwareMap, Telemetry telemetry) {
        ...
        // 라임라이트 3A 하드웨어 매핑 (Driver Station에서 디바이스 이름을 'limelight'로 등록해야 합니다)
        limelight = hardwareMap.get(Limelight3A.class, "limelight");
        
        // 라임라이트 설정
        limelight.setPollRateHz(100); // 초당 데이터 갱신 횟수 (100Hz로 매우 빠르게)
    ...
    public void update() {
        ...
        if (result != null && result.isValid()) {
            ...
            ty = 0.0;
            ta = 0.0;
            telemetry.addData("Vision Target", "❌ 없음");
        }
    }
